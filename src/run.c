...
static char const rcsid[] = "@(#)$Id: run.c 1404 2008-03-08 23:25:46Z kalt $";
...
static int
mailsubject(FILE *stream, int cf)
{
    char *format;
    int err = 1;
        ...
        else
          {
            ...
            switch (*format)
              ...
              case 'h':
                  ...
                  break;
              case 'i':
                  if (*id != '\0') err = fprintf(stream, "%s", id);
                  break;
              case 'n':
...
tail(void *fdarray)
{
    ...
    if (ptr < fd[2])
	fprintf(stderr, "%s: Standard %s truncated (%d bytes out of %d).\n",
		jid, (fd[1] == fileno(stdout)) ? "output":"error", ptr, fd[2]);
    pthread_exit(NULL);
}
#endif
...
#define SM_LOCK    2
static void
send_notification(int type, char *to, pid_t pid, int ptr)
{
...
void
run(char *cfdir, char *myjob, char *myid, char *slfac, int options, char *envp[])
{
    struct sigaction sa;
	...
	if (ttyname(fileno(stdin)) != NULL)
		...
		jid, (pw != NULL && pw->pw_name != NULL) ? pw->pw_name : "?",
	...
	log(LOG_INFO, "%s: stdout %s %ld %s", jid,
	    mystrftime("%Y/%m/%d %H:%M:%S", out_timestamp), out_size, out_md5);
    if (err_timestamp != 0)
	log(LOG_INFO, "%s: stderr %s %ld %s", jid,
	    mystrftime("%Y/%m/%d %H:%M:%S", err_timestamp), err_size, err_md5);
	...
	if ((options & RUN_FAST) == 0)
	  {
	    ...
	    debug(DINFO, "Sleeping %lu (< %lu) seconds before starting...",
		  delay, cf_getnum(CF_RANDOMDELAY));
            state_delay(delay);
	    log(LOG_INFO, "%s: Delaying start by %s", jid, unit_rtime(delay));
		...
		error("sigaction(SIGHUP, handler, NULL) failed: %s", ERRSTR);
	    if (sigaction(SIGINT, &sa, NULL) != 0)
		error("sigaction(SIGINT, handler, NULL) failed: %s", ERRSTR);
	    if (sigaction(SIGQUIT, &sa, NULL) != 0)
		error("sigaction(SIGQUIT, handler, NULL) failed: %s", ERRSTR);
	    if (sigaction(SIGTERM, &sa, NULL) != 0)
		error("sigaction(SIGTERM, handler, NULL) failed: %s", ERRSTR);
	...
	else
	  {
	    ...
	    log(LOG_INFO, "%s: \"randomdelay\" option disabled by user", jid);
	  }
      }
    ...
    if (tmpstr != NULL)
      {
        ...
        action = strtok_r(tmpstr, ",", &last); loop = NULL;
        if (cf_getstr(CF_LOCKFILE) == NULL)
            snprintf(fname, PATH_MAX, "%s/.%s-%s", cfdir, jid, get_hostname(0));
        else
        ...
        while ((lockfd = open(fname, O_WRONLY|O_CREAT|O_EXCL, 0444)) < 0)
          {
            ...
            if (errno != EEXIST)
              {
                error("Failed to create lock file (%s): %s", fname, ERRSTR);
            ...
            if (lockfd >= 0)
              {
                ...
                if (mapfile(fname, lockfd, (void **) &locked, &lockedlen) != 0)
                  {
                        ...
                        if (errno == ESRCH)
                          {
                            ...
                            log(LOG_WARNING, "%s: Breaking apparently stale lock (%s) left by process id %ld", jid, fname, (long) lockedby);
                            if (unlink(fname) == 0)
                                continue;
                            if (time(NULL) > when)
                              {
                                error("Failed to break stale lock (%s): %s",
                                      fname, ERRSTR);
                                when = time(NULL) + 300;
                              }
                          }
                        else
                          {
                            ...
                            state_close("Aborted: fatal lock check failure");
                            exit(1);
                          }
                      }
                  }
        ...
        if (lockfd >= 0)
          {
            ...
            if (write(lockfd, mypid, strlen(mypid) + 1) != strlen(mypid) + 1)
              {
                error("Failed to write process id in new lock file (%s): %s",
                      fname, ERRSTR);
              }
            else
	...
	exit(1);
      ...
      {
	tails[0] = outfd; tails[1] = fileno(stdout); tails[2] = -1;
	...
	if (errfd < 0)
	  ...
	  {
	    tails[3] = errfd; tails[4] = fileno(stderr); tails[5] = -1;
	    status = pthread_create(&errthd, NULL, tail, (void *) &tails[3]);
	    if (status != 0)
    ...
    /* Initialize timeout */
    start = time(NULL);
    tmpstr = cf_getstr(CF_TIMEOUT); last = NULL; loop = NULL;
    if (tmpstr != NULL)
      {
	tmpstr = strdup(tmpstr);
	if (tmpstr == NULL)
	...
	if (end != 0 && time(NULL) > end)
	  {
		...
		if (isdigit((int) action[0]))
		  ...
		  {
		    debug(DINFO, "Sending signal %d to child process group",
			  atoi(action+1));
                    state_killed();
			...
			log(LOG_WARNING, "%s: Failed to send signal %d to child process group: %s",
			    jid, atoi(action+1), ERRSTR);

		  }
		else if (strncmp(action, "SIG", 3) == 0)
		  {
		    debug(DINFO, "Sending signal %s to child process group",
			  action);
                    state_killed();
			...
			log(LOG_WARNING, "%s: Failed to send signal %s to child process group: %s",
			    jid, action, ERRSTR);
		  }
		else if (action[0] == '=' && isdigit((int) action[1]))
		  {
    ...
    if (utimestr == NULL)
      {
	error("strdup(%s) failed: %s", unit_rtime(ru.ru_utime.tv_sec), ERRSTR);
	utimestr = "??";
      }
    stimestr = strdup(unit_rtime(ru.ru_stime.tv_sec));
    if (stimestr == NULL)
      {
	error("strdup(%s) failed: %s", unit_rtime(ru.ru_stime.tv_sec), ERRSTR);
	stimestr = "??";
      }
    ...
    if (WIFEXITED(status))
      {
	debug(DINFO, "Normal termination, returned %d", WEXITSTATUS(status));
	log(LOG_INFO,
	    "%s: Command returned %d after %s (%s user, %s sys)",
	    jid, WEXITSTATUS(status), unit_rtime(end - start),
	    utimestr, stimestr);
	...
	if (err > 0 && cf_getrptstr(rpt, CF_RPTSUBJECT) != NULL)
	  {
	    ...
	    debug(DINFO, "-> Subject: %s", cf_getrptstr(rpt, CF_RPTSUBJECT));
	  }
	if (err > 0)
	...
	while (1)
	  {
		...
		if (errno == EINTR)
		    ...
		    error("waitpid(%d[sendmail for report \"%s\"]) failed: %s",
	...
	if (WIFSIGNALED(smstatus))
	    error("sendmail for report \"%s\" died from %s",
		  cf_getrptstr(rpt, CF_RPTNAME), strsignal(WTERMSIG(smstatus)));
      }
