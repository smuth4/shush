...
static int getnext(char *cfdname, char **schedule, char **job, char **id)
{
	...
	*schedule = cf_getstr(CF_SCHEDULE);
	...
	if ((nl - calendar) == clen) {
		...
		if (lndup == NULL) {
		    fprintf(stderr, "%s: malloc() failed: %s", myname,
			    ERRSTR);
		    exit(1);
		}
	...
	ln = nl + 1;
	if (strcmp(token, "*") != 0
	    && strcasecmp(token, get_hostname(0)) != 0)
	    return getnext(cfdname, schedule, job, id);
...
void crontab(char *mypath, char *cfdir, int action)
{
	...
	if (unlink(newtab) == 0)
	    exit(0);
	error("Failed to remove temporary file \"%s\": %s", newtab,
	      ERRSTR);
	exit(1);
    }
    ...
    if (WIFEXITED(status) && WEXITSTATUS(status) != 0)
	error("\"crontab %s\" returned %d?!", argv[1],
	      WEXITSTATUS(status));
    else if (WIFSIGNALED(status) || WIFSTOPPED(status))
